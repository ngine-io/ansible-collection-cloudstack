...
.. code-block:: yaml
    ...
    - name: ensure my ssh public key exists in all CloudStack regions
      local_action: ngine.cloudstack.ssh_key
        name: my-ssh-key
        public_key: "{{ lookup('file', '~/.ssh/id_rsa.pub') }}"
        api_region: "{{ item }}"
        loop:
...
.. code-block:: yaml
    ...
    ---
    ngine_io.cloudstack.offering: Small
...
.. code-block:: yaml
    ...
    ---
    ngine_io.cloudstack.offering: Large
        ...
        - name: run all enclosed tasks from localhost
          ...
          block:
            ...
            - name: ensure static NATs
              ngine_io.cloudstack.static_nat: vm="{{ inventory_hostname_short }}" ip_address="{{ public_ip }}"
              when: public_ip is defined
...
.. Note:: For some modules, for example ``cs_ssh_key`` you usually want this to be executed only once, not for every VM. Therefore you would make a separate play for it targeting localhost. You find an example in the use cases below.
...
.. code-block:: yaml
    ...
    ---
    ngine_io.cloudstack.offering: Small
    ngine_io.cloudstack.securitygroups: [ 'default']
...
.. code-block:: yaml
    ...
    # file: group_vars/webserver
    ---
    ngine_io.cloudstack.securitygroups: [ 'default', 'web' ]

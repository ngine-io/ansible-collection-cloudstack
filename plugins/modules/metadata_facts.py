...
- name: Gather cloudstack facts
  ngine_io.cloudstack.metadata_facts:

# Gather specific fact on instances
- name: Gather cloudstack facts
  ngine_io.cloudstack.metadata_facts: filter=cloudstack_instance_id
...
class CloudStackFacts(object):
    ...
    def __init__(self):
        collector = ansible_collector.get_ansible_collector(
            all_collector_classes=default_collectors.collectors, filter_spec="default_ipv4", gather_subset=["!all", "network"], gather_timeout=10
        )
...
def main():
    ...
    metadata_facts = CloudStackFacts().run()
    metadata_facts_result = dict(changed=False, ansible_facts=metadata_facts)
    module.exit_json(**metadata_facts_result)

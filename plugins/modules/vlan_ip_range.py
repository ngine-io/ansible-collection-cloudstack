...
def main():
    ...
    module = AnsibleModule(
        ...
        mutually_exclusive=(["account", "project"],),
        required_if=(("state", "present", ("gateway", "netmask")),),
        supports_check_mode=True,
    )

    avlan_ip_range = AnsibleCloudStackVlanIpRange(module)

    state = module.params.get("state")
    if state == "absent":
        ipr = avlan_ip_range.absent_vlan_ip_range()

    else:
        ipr = avlan_ip_range.present_vlan_ip_range()

    result = avlan_ip_range.get_result(ipr)
